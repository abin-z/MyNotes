...
void test01()
{
	...
	char c1 = 'a';
	char c2 = 'b';
	myPrint(c1, c2); //调用函数模板, 因为调用函数模板可以不用隐式类型转换
}
...
public:
	...
	MyArray& operator=(const MyArray& myarray) {
		// 自我赋值检测  一定要记得写, 不仅仅是为了效率, 也是为了正确性
        if(this == &myarray){
            return *this;
        }
	...
	void Pop_back()
	{
		...
		this->m_Size--;		// 逻辑删除,数据任然在原来的位置上,但是无法访问
	}
...
质变算法：是指运算过程中==会更改==区间内的元素的内容。例如拷贝，替换，删除等等

非质变算法：是指运算过程中==不会更改==区间内的元素内容，例如查找、计数、遍历、寻找极值等等
...
注意: [C++ 中的中文编码](https://zhuanlan.zhihu.com/p/234893634)

**中文字符在C++中占的字节数和英文不一样.**

中文字符在计算机中的编码。中文和英文不同，英文26个字母就能组合各种词汇，但是中文有很多汉字，因此，计算机中对中文需要编码，也就是用多个字节表示一个汉字。计算机中中文有多种编码方式，比如ANSI编码，unicode编码。根据资料，unicode貌似只是编码方式，而不是实现方式。它有多种实现方式，比如UTF-8,UTF-16等。其中常用的UTF-8是不定长编码，也就是说字符可能只有一个字节，比如英文字符，也可能有2-4个字节，比如汉字。其中英文字符的码值和ASCII一致，都小于128.
既然汉字等字符是多字节组成的，那么诸如
string str="hello你好宝贝";
这样的字符串，想要截取“你好”两个汉字就稍微复杂一些，肯定不能用substr(5,2)这样子，因此长度2只是字节数，不是一个汉字真是的字节数，一个汉字可能2个字节，也可能4个字节。
...
* find找到字符串后返回查找的**第一个字符位置**，找不到返回-1
...
* `swap(vec);`  // 将vec与本身this的元素互换

resize()后可以实现收缩内存 `vector<int>(v).swap(v); //匿名对象`(调用拷贝构造初始化匿名对象)  会释放原来的空间.
...
![image-20220928224655630](https://my-pic-bed.oss-cn-chengdu.aliyuncs.com/typora_picture/image-20220928224655630.png)

调用`vector<int>(v).swap(v);`交换后: (匿名对象调用完后会立即被释放空间)

![image-20220928225109796](https://my-pic-bed.oss-cn-chengdu.aliyuncs.com/typora_picture/image-20220928225109796.png)
...
**概念：**Queue是一种**先进先出**(First In First Out,FIFO)的数据结构，它有一个入口和一个出口.
...
由于链表的存储方式并不是连续的内存空间，因此链表list中的迭代器只支持前移和后移，不支持随机访问, 属于**双向迭代器**
...
* insert(pos,elem);       //在pos位置(迭代器)插elem元素的拷贝，返回新数据的位置。
* insert(pos,n,elem);   //在pos位置(迭代器)插入n个elem数据，无返回值。
* insert(pos,beg,end);//在pos位置(迭代器)插入[beg,end)区间(迭代器)的数据，无返回值。
* clear();                       //移除容器的所有数据
* erase(beg,end);       //删除[beg,end)区间(迭代器)的数据，返回下一个数据的位置。
* erase(pos);               //删除pos位置(迭代器)的数据，返回下一个数据的位置。
...
void test01()
{
	...
	//查找
	set<int>::iterator pos = s1.find(30);	 // 没找到返回.end()迭代器
...
{
	set<int> s;
	pair<set<int>::iterator, bool>  ret = s.insert(10);	//返回的是一个pair对组包含迭代器和是否插入成功
	if (ret.second) {
		cout << "第一次插入成功!" << endl;
	}
	else {
...
- **对于自定义数据类型，set必须指定排序规则(仿函数)才可以插入数据**
