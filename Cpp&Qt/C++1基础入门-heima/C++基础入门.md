...
在计算机内，**有符号数**有3种表示法：原码、反码和补码。所有数据的运算都是采用**补码**进行的。    

**原码 反码 补码 对于**有符号数值而言:

1. **二进制的最高位是符号位**: 0表示正数, 1表示负数.
2. **正数的原码,反码,补码都一样** (三码合一).
3. **负数的反码 = 原码的符号位不变,其它位按位取反**(0-->1,1-->0).
4. **负数的补码 = 它的反码 + 1;**    负数的反码 = 负数的补码 - 1;
5. **0的反码,补码都是0;**
6. **Java没有无符号数**,换言之,Java中的数值都是有符号数, 但是C/C++有无符号数
7. **在计算机运算的时候,都是以==补码==的方式**来运算的.
...
数据类型存在的意义:    给该类型的变量分配合适大小的内存空间.

![image-20220805005819360](https://my-pic-bed.oss-cn-chengdu.aliyuncs.com/typora_picture/image-20220805005819360.png)
...
> 总结：在除法运算中，除数不能为0; 两个整数相除结果依然是整数
...
> 总结：只有**整型**变量可以进行取模运算, 求余数; 取模运算中除数也不能为0
...
![image-20220808233245362](https://my-pic-bed.oss-cn-chengdu.aliyuncs.com/typora_picture/image-20220808233245362.png)
...
![image-20220808224132667](https://my-pic-bed.oss-cn-chengdu.aliyuncs.com/typora_picture/image-20220808224132667.png)
...
![image-20220815223254517](https://my-pic-bed.oss-cn-chengdu.aliyuncs.com/typora_picture/image-20220815223254517.png)
...
**所有指针类型在32位操作系统下是4个字节, 在64位操作系统下为8个字节**
...
> 总结2：创建结构体变量时，关键字struct可以省略, C语言不可省略
...
**作用：**用const来防止误操作, 添加const之后在函数体内部则不能修改值(一般用于地址传递和引用传递)
